...
- PDF, plain text, Markdown; ≤ 20 MB; ≤ 12 per lecture, ≤ 30 per class. Same presign → browser PUT → server confirm contract as recordings, under the owner's R2 prefix, owner/purpose stamped in metadata and checked on confirm.
- **Two scopes, one table (since 18 Sep).** A request names `lecture_id` (the file belongs to that lecture; the row carries its class too) or `class_id` (the file belongs to the course — syllabus, formula sheet, past exam — and `lecture_id` is null; migration `20260918120000_class_level_materials.sql`). `class_id` is always set, so every file cascades with its class. The lecture page lists and uploads in lecture scope; the class page's **Course materials** widget (same component, `cls` prop) lists every file of the class, lecture files labelled and linked, and uploads in class scope. A class file feeds no lecture's analysis: the enrichment pass (`loadLectureMaterials`) reads only a lecture's own files, so a course-wide deck never verifies one lecture's formulas. What reads class files is the study-material generator, below.
- **Practice material from files (since 18 Sep).** `POST /generateStudyMaterial` takes `material_ids` — any readable files of the class, course-level or lecture-attached, chosen by the student under the lecture picker on the Study page (`MaterialScopePicker`) — and reads their extracted text beside the lecture summaries; `no_lectures: true` builds from files alone (a past exam is enough). The prompt lives in `server/lib/studyMaterial.js`: files add a second source block within `STUDY_MATERIALS_CHARS` (60k, shortest file first so a formula sheet is never crowded out by a chapter) and change the one phrase that names the sources; a request without `material_ids` produces the prompt it always did, byte for byte (the test holds a frozen copy). Files from another class or account are not found rather than refused. Same `study_material` gate and the same one credit with or without files — the PDF read was charged at upload and is not charged again. The response names the files actually read (`materials_used`).
- Text extraction: the cheap Gemini model reads the PDF inline (the same call shape `parseTimetableUpload` uses for timetable images) and returns the text with page markers (`[Page 3]`), formulas in plain linear notation, tables row by row. No PDF library in the server; scanned slides and photographed handouts are read too (OCR is free with a multimodal model). A PDF read costs the student one credit (`material_extract`, Student and up, charged only after a successful read); text and Markdown are gated on tier but free. A file that yields no text is kept for download and marked `failed` so the UI can say why it is not used for verification.
- Client: `base44.materials.upload(target, file)` where `target` is a lecture id or `{ lecture_id }` / `{ class_id }`, plus `getDownloadUrl / delete`; rows are read through `entities.LectureMaterial` (RLS, select only). Only the API writes rows, so a row can never point at an unchecked object.
- Lecture, class and semester deletion remove the objects (class and semester deletes collect refs by class, so class-level files go too); data export includes the table.
...
- **Class files in enrichment.** A course file is read by the study-material generator but not by any lecture's enrichment pass (deliberately — a whole deck verifying one lecture's formulas is more prompt and more noise for every recording). An opt-in per class, if students ask for it.
- **Materials rows carry their text to the browser.** `entities.LectureMaterial` selects `*`, so every list of a class's files (class page, lecture page, Study page) downloads `extracted_text` the client never reads — up to 120k characters a file. A column list for that table in `cedarClient` is the fix.
