...
### Addendum, 2 Sep — two pipelines build every push (corrects the 1 Sep note)

On 1 Sep this section said praelecta.ca was pinned to an old Worker version and that frontend deploys were not reaching users. That was wrong. The served bundle matched no CI deploy because **Cloudflare's own Git integration (Workers Builds) also builds every commit** and deploys its version a few minutes after GitHub Actions does, so its version is always the active one. It is the same source — the live bundle contains the 1 Sep island changes — built with a dashboard variable that sets `VITE_RENDER_API_URL` to the onrender host, which is why the hashes never matched and why the CSS hash *did* match. Users received every deploy.

Fix: the deploy workflow now runs `wrangler versions deploy <id>@100%` after upload and then fails if praelecta.ca does not serve the bundle it just built (commit cba37f2) — that check is what exposed the second builder. Remaining action: disconnect the Workers Builds Git integration in the Cloudflare dashboard so one pipeline owns the site. The CSP fix from 1 Sep stands: the CI build points at `api.praelecta.ca` and needed it.
