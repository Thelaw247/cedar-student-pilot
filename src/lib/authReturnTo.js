...
// Resolve ?returnTo= to a safe same-origin path, else the authenticated Today route.
//
...
export function safeReturnTo() {
  ...
  try {
    ...
    if (url.origin !== window.location.origin) return "/today";
    ...
    if (!path.startsWith("/") || path.startsWith("//") || path.includes("\\")) return "/today";
    return path;
  } catch {
    return "/today";
  }
