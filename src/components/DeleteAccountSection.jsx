...
/**
 * Account deletion. Lives in the Account section next to profile/sign-out —
 * this is a decision about the ACCOUNT, not a data-management action, so it
 * moved out of Data & Privacy (which still holds export + the privacy policy
 * link). Logic is unchanged from before the move.
 */
export default function DeleteAccountSection() {
  const { clearOfflineData, logout } = useAuth();

  // `confirming` shows the destructive confirmation panel; the user must type
  // the confirmation word before the delete button enables.
