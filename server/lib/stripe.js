...
import { TIER_GRANT, periodKey } from './credits.js';
...
// base44_app_id) as the live Base44 checkout-session creator, so this
// endpoint can be tested against real Stripe test-mode events before
// anything else in the migration is live.
    ...
    if (existing.rows.length > 0) {
      await pool.query(
        `update processed_stripe_events set
           user_id = $2, kind = $3, stripe_event_id = $4, stripe_session_id = $5,
           credits_granted = $6, status = $7, last_error = $8,
      ...
      await pool.query(
        `insert into processed_stripe_events
           (user_id, anchor_id, kind, stripe_event_id, stripe_session_id, credits_granted, status, last_error, attempt_count, completed_at, processed_at)
